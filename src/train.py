...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False,factor=0.1, patience=10, version="randomizedsearch"):
    # initialize tracker for minimum validation loss
    if interactive_tracking:
        liveloss = PlotLosses(outputs=[MatplotlibPlot(after_subplot=after_subplot)])
    else:
        liveloss = None
    if version != "randomizedsearch":
        try:
            content = torch.load(f"./checkpoints/best_val_loss{version}.pt")
            valid_loss_min = content["valid_loss"]
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if valid_loss_min is None or (
                (valid_loss_min - valid_loss) / valid_loss_min > 0.005
        ):
            ...
            content["lr"] = optimizer.param_groups[0]["lr"]
            content["valid_loss"] = valid_loss
